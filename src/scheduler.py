...
class ScenarioScheduler:
    ...
    def _execute_test(self, scenario_id: str, scenario_config: dict) -> None:
        ...
        try:
            ...
            log_error("scheduler", "_execute_test", e,
                      context=f"scenario={scenario_id}, protocol={protocol}")
        finally:
            # Always signal completion for "once" mode so the worker doesn't hang
    ...
    def finalize_scenario(self, scenario_id: str) -> None:
        ...
        # Clean up in-memory tracking for this scenario to prevent memory leaks
        self._cleanup_scenario(scenario_id)

    def _cleanup_scenario(self, scenario_id: str) -> None:
        """Remove all in-memory tracking data for a completed scenario."""
        self.scenario_configs.pop(scenario_id, None)
        self.scenario_end_times.pop(scenario_id, None)
        self.scenario_jobs.pop(scenario_id, None)
        self.completion_events.pop(scenario_id, None)
        with self._running_jobs_lock:
            self._running_jobs.pop(scenario_id, None)
