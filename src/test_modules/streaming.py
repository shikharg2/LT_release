...
def _run_parallel_streaming(playback_urls: list, headless: bool, disable_cache: bool,
    ...
    results = []
    with ThreadPoolExecutor(max_workers=min(len(playback_urls), 8)) as executor:
        future_to_url = {
...
def _monitor_full_playback(page, metrics: dict) -> None:
    ...
    MAX_MONITOR_SECONDS = 7200  # 2 hour hard ceiling to prevent infinite hangs

    while True:
        if time.time() - monitor_start > MAX_MONITOR_SECONDS:
            print(f"[streaming] Monitor timeout: exceeded {MAX_MONITOR_SECONDS}s ceiling, ending monitoring")
            break
